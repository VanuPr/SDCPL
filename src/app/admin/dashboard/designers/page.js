...
export default function AdminDesigners() {
  ...
  const fetchLeads = async () => {
    try {
      const q = query(collection(db, "designer_leads"), orderBy("timestamp", "desc"));
      const querySnapshot = await getDocs(q);
      const data = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      setLeads(data);
    } catch (err) {
      console.error("Error fetching designer leads:", err);
    }
    setLoading(false);
  };
  ...
  const handleDelete = async (id) => {
    if (confirm("Are you sure you want to delete this application?")) {
      try {
        await deleteDoc(doc(db, "designer_leads", id));
        fetchLeads();
      } catch (err) {
  ...
  return (
    <div className="p-8">
      <h1 className="text-2xl font-bold text-gray-800 mb-6">Designer Applications</h1>
              ...
              {leads.map((lead) => (
                <tr key={lead.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-blue-600 hover:underline">
                    <a href={lead.portfolio} target="_blank" rel="noopener noreferrer">View Portfolio</a>
                  </td>
